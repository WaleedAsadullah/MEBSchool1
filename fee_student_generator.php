...
$students = query_to_array("SELECT * FROM `ad_assign_student_class` WHERE `gr_no` = $student_id");
...
if($ok_to_print)  $class_id = $students[0]['assign_class'];
...
$generate_fee = query_to_array("SELECT * FROM `ac_generate_fee_student` WHERE `student_id` = $student_id and `which_month` = \"$generate_fee_for_month\" and `year` = \"$generate_fee_for_year\"");
...
echo "<br>";
if(isset($generate_fee) && (is_numeric($generate_fee[0]['other'])) )
echo "Other fee(actual)=".$generate_fee[0]['other'];
else $generate_fee[0]['other'] = 0;
echo "<br>";
...
echo "<br>";
$fee = $monfee + $admfee +$examfee +$miscfee +$specialfee +$annualfee -$feesibdisc -$feeza +$generate_fee[0]['other'];
